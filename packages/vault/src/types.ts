...
export type AdapterHooks = {
  beforeRead?: (record: MarkdownRecord) => MarkdownRecord | Promise<MarkdownRecord>;
  afterRead?: (record: MarkdownRecord) => MarkdownRecord | Promise<MarkdownRecord>;
  beforeWrite?: (record: MarkdownRecord) => MarkdownRecord | Promise<MarkdownRecord>;
  afterWrite?: (record: MarkdownRecord) => MarkdownRecord | Promise<MarkdownRecord>;
  beforeSync?: (records: MarkdownRecord[]) => MarkdownRecord[] | Promise<MarkdownRecord[]>;
...
export type BaseSubfolderMethods<TSchema extends SchemaDefinition> = {
  ...
  update(id: string, updates: Partial<InferRecord<TSchema>>): Promise<InferRecord<TSchema>>;
...
export type VaultCoreMethods = {
  ...
  $stats(): Promise<{ subfolders: number; totalRecords: number; lastSync: Date | null }>;
...
export type BuildVaultType<TAdapters extends readonly AdapterConfig[]> = {
  [K in keyof UnionToIntersection<ExtractAllSubfolders<TAdapters>>]: 
    K extends string
